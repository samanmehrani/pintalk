...
import { isBuildTime } from '@/api/runtime/build'

const envSchema = z.object({
  DATABASE_URL: z.string().min(1, 'DATABASE_URL is required'),
  REDIS_URL: z.string().min(1, 'REDIS_URL is required'),
  JWT_SECRET: z.string().min(16, 'JWT_SECRET must be at least 16 characters'),
  JWT_ISSUER: z.string().min(1).default('pintalk.local'),
  JWT_AUDIENCE: z.string().min(1).default('pintalk.app'),
  TOKEN_DEFAULT_TTL_HOURS: z.coerce.number().int().positive().default(24),
  APP_NAME: z.string().min(1).default('Pintalk'),
  PUBLIC_BASE_URL: z.string().min(1).default('http://localhost:3020'),
  ...
  OTP_FIXED_CODE: z.string().min(4).default('12345'),
